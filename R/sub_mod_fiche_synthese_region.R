#' fiche_synthese_region UI Function
#'
...
mod_fiche_synthese_region_ui <- function(id, current_year_fr, current_year_vr){
  ns <- NS(id)
  tagList(
    br(),
...
#' fiche_synthese_region Server Functions
#'
#' @noRd
mod_fiche_synthese_region_server <- function(id, validated_selection, current_year_ac, current_year_st,
  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      # Introduction panel
      mod_intro_panel_server_fiche_synthese("intro_fs", validated_selection, data_cat_terr, admin_region)
      ...
      data_stationnement <- reactive(stationnement_regional |> filter(INSEE_REG == validated_selection$region))
      data_accueil_velo <- reactive(accueil_velo_regional |> filter(INSEE_REG == as.numeric(validated_selection$region)))

      # Radar chart
      output$radar_chart <- renderPlotly({
        data <- generate_data_radar_chart(data_cat_terr, amenagement_regional, stationnement_regional,
                                          part_modale, veloroute_regional, accueil_velo_regional,
                                          current_year_ac, current_year_st, current_year_pm,
                                          current_year_vr, current_year_av, "region", validated_selection$region)
      ...
      comparison_data <- reactive(generate_comparison_data_local(data_cat_terr, amenagement_regional, "region",
                                                                 "taux_cyclabilite", validated_selection$region, seq(2021, current_year_ac)))
      ...
      output$evolution_taux_cyclabilite <- renderPlotly({
        ...
        plot_evolution_amenagement(data_amenagement(), data_comparison_cat, "taux_cyclabilite",
                                   territory_name(), seq(2021, current_year_ac), "region", lib_category()) |>
      ...
      output$evolution_voirie_hab <- renderPlotly({
        comparison_data <- generate_comparison_data_local(data_cat_terr, amenagement_regional, "region",
                                                          "voirie_hab", validated_selection$region, seq(2021, current_year_ac))
      ...
      output$legend_evolution_ac <- renderUI({
        legend_histo_fiche_synthese(territory_name(), paste0("Maximum des ", lib_category()),
                                    paste0("Minimum des ", lib_category()), paste0("Moyenne pondérée des ", lib_category()))
        })
      ...
      output$evolution_stat_hab <- renderPlotly({
        comparison_data <-generate_stationnement_comparison_data_local(data_cat_terr, stationnement_regional, "stat_hab",
                                                                       "region", validated_selection$region, 2022, current_year_st)

        data_comparison_cat <- comparison_data$data_comparison_cat

        plot_evolution_stationnement(data_stationnement(), data_comparison_cat, "stat_hab",
                                     territory_name(), 2022, current_year_st, "region", lib_category()) |>
          layout(showlegend = FALSE, title = list(text = ""))
      })

      # Evolution - Stationnement par km d'aménagement
      output$evolution_stat_km <- renderPlotly({
        comparison_data <- generate_stationnement_comparison_data_local(data_cat_terr, stationnement_regional, "stat_km",
                                                                        "region", validated_selection$region, 2022, current_year_st)
      ...
      output$evolution_part_modale <- renderPlotly({
        data_evolution <- generate_pm_comparison_data_local(data_cat_terr, part_modale, current_year_pm, 2018,
                                                            "velo", "region", validated_selection$region)
      ...
      # Value boxes
      vb_pm_content <- reactive(value_box_content_pm(part_modale, current_year_pm, "region", validated_selection$region, "velo"))
      ...
      # Value boxes
      vb_av_content<- reactive(value_box_content_accueil_velo(data_accueil_velo(), presta_types_table(), "region", current_year_av,
                                                              "count", "TOTAL", NULL))
      ...
      output$plot_completion_ev <- renderPlotly({
        plot_avancement_veloroutes(veloroute_regional, current_year_vr, "EV",
                                   "region", validated_selection$region, territory_name(), "fs")
      })

      output$plot_completion_snv <- renderPlotly({
        plot_avancement_veloroutes(veloroute_regional, current_year_vr, "SNV-STRICT",
                                   "region", validated_selection$region, territory_name(), "fs")
      })

      output$plot_completion_srv <- renderPlotly({
        plot_avancement_veloroutes(veloroute_regional, current_year_vr, "SRV-STRICT",
                                   "region", validated_selection$region, territory_name(), "fs")
      })

      output$plot_completion_sdv <- renderPlotly({
        plot_avancement_veloroutes(veloroute_regional, current_year_vr, "SDV-STRICT",
                                   "region", validated_selection$region, territory_name(), "fs")
      })

      # Value box - Total number of kilometers
      vb_vr_content <- reactive(value_box_content_veloroute(veloroute_regional, current_year_vr, "region",
      ...
      # Passages par semaine
      weekly_passages <- reactive(plot_weekly_passages(freq_daily, current_year_fr, "region", "region", territory_name()))
      ...
      output$vb_fr_chiffre_cle <- renderText({
        evol <- table_avg_daily_traffic(freq_global_means, current_year_fr, "region",
                                        validated_selection$region, territory_name())$evol
        ...
        card_title(
          HTML(
            paste0(
              ...
              "<span style='font-size:0.8rem; color:#555;'>",
              territory_name(),
              " - À échantillon comparable, base ",
              freq_global_evol |> filter(cog == validated_selection$region) |> pull(nb_compteurs),
              " compteurs",
              "</span>"
            )
          )
      ...
      output$global_evolution <- renderPlotly({
        ...
        plot_global_evolution(data, 2019, current_year_fr, "region",
...
# mod_fiche_synthese_region_ui("fiche_synthese_region_1")

## To be copied in the server
# mod_fiche_synthese_region_server("fiche_synthese_region_1")
