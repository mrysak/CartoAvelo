#' part_modale_departement UI Function
#'
...
mod_part_modale_departement_ui <- function(id, current_year){
  ns <- NS(id)
  tagList(
    br(),
    ...
    card(
      card_title(paste0("Distribution par type de commune des parts modales des différents modes de transport en ", current_year), class = "h4"),
      card_body(
        layout_columns(
          col_widths = c(8, 4),
          div(
            style = "margin-left: 10%;",
            plotlyOutput(ns("pm_by_milieu"))
          ),
          # Legend
          div(
...
#' part_modale_departement Server Functions
#'
#' @noRd
mod_part_modale_departement_server <- function(id, validated_selection, current_year, admin_cache) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$map_part_modale <- renderLeaflet({
        ...
        map_part_modale(data_cat_terr, part_modale, admin_sf, "departement",
                        input$display_typology, validated_selection$departement, input$type_transport,
                        input$info_to_display, current_year, admin_cache)
      })

      value_box_content <- reactive(value_box_content_pm(part_modale, current_year, "departement", validated_selection$departement, input$type_transport))
      ...
      output$evolution_part_modale <- renderPlotly({
        data_evolution <- generate_pm_comparison_data_local(data_cat_terr, part_modale, current_year, 2018,
                                                            input$type_transport_evol, "departement", validated_selection$departement)
      ...
      output$pm_by_milieu <- renderPlotly({
        data_plot <- pm_distribution_by_milieu(part_modale, data_cat_terr, current_year, "departement", validated_selection$departement)
        plot_pm_distribution_by_milieu_departement(data_plot)
      })

      # Distribution by gender
      output$pm_by_gender <- renderPlotly({
        plot_distrib_gender_pm(distrib_gender_pm, data_cat_terr, "departement", validated_selection$departement, territory_name())
      })

      # Modal share by age group
      output$pm_by_age_velo <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "departement", validated_selection$departement, territory_name(), "velo")
      })

      output$pm_by_age_pied <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "departement", validated_selection$departement, territory_name(), "pied")
      })

      output$pm_by_age_tcommun <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "departement", validated_selection$departement, territory_name(), "tcommun")
      })

      output$pm_by_age_car <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "departement", validated_selection$departement, territory_name(), "car")
      })
...
# mod_part_modale_departement_ui("part_modale_departement_1")

## To be copied in the server
# mod_part_modale_departement_server("part_modale_departement_1")
