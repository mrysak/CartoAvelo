#' part_modale_france UI Function
#'
...
mod_part_modale_france_ui <- function(id, current_year){
  ns <- NS(id)
  tagList(
    br(),
...
#' part_modale_france Server Functions
#'
#' @noRd
mod_part_modale_france_server <- function(id, validated_selection, current_year, admin_cache) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$map_part_modale <- renderLeaflet({
        ...
        map_part_modale(data_cat_terr, part_modale, admin_sf, "france",
                        input$display_typology, "", input$type_transport,
                        input$info_to_display, current_year, admin_cache, session$ns)
      })

      value_box_content <- reactive(value_box_content_pm(part_modale, current_year, "france", "", input$type_transport))
      ...
      output$evolution_part_modale <- renderPlotly({
        data_comparison_cat <- generate_pm_comparison_data_national(data_cat_terr, part_modale,
                                                            current_year, 2018, input$type_transport_evol)

        plot_evolution_part_modale(part_modale |> filter(insee_niveau == "NAT"), data_comparison_cat,
                                   input$type_transport_evol, 2018, current_year,
                                   "France entière", "france", "")
      })

      # Distribution by commune density
      output$pm_by_milieu <- renderPlotly({
        data_plot <- pm_distribution_by_milieu(part_modale, data_cat_terr, current_year, "france", "")
        plot_pm_distribution_by_milieu_departement(data_plot)
      })

      # Distribution by gender
      output$pm_by_gender <- renderPlotly({
        plot_distrib_gender_pm(distrib_gender_pm, data_cat_terr, "france", "", "France entière")
      })

      # Modal share by age group
      output$pm_by_age_velo <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "france", "", "France entière", "velo")
      })

      output$pm_by_age_pied <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "france", "", "France entière", "pied")
      })

      output$pm_by_age_tcommun <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "france", "", "France entière", "tcommun")
      })

      output$pm_by_age_car <- renderPlotly({
        plot_distrib_age_pm(distrib_age_pm, data_cat_terr, "france", "", "France entière", "car")
      })
...
# mod_part_modale_france_ui("part_modale_france_1")

## To be copied in the server
# mod_part_modale_france_server("part_modale_france_1")
