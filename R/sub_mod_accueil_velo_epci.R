...
mod_accueil_velo_epci_ui <- function(id, current_year){
  ns <- NS(id)
  tagList(
    br(),
...
#' accueil_velo_epci Server Functions
#'
#' @noRd
mod_accueil_velo_epci_server <- function(id, validated_selection, current_year) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      filtered_data <- reactive(accueil_velo_epci |> filter(INSEE_EPCI == validated_selection$epci))

      # Introduction panels
      mod_intro_panel_server("intro_av", "accueil_velo",
                             validated_selection, data_cat_terr,
                             accueil_velo_epci, "accueil_velo_data")

      # Count of Accueil Vélo services - Declined by category
      output$table_services_by_category <- render_gt({

        table_services <- prepare_dep_epci_com_data(
          accueil_velo_epci, accueil_velo_national, data_cat_terr,
          "INSEE_EPCI", "EPCI", "LIBEPCI",
          validated_selection$epci,
          current_year, presta_types_table()
        )

        generate_services_count_table(table_services, "epci", color_low = "#F6F9ED", color_high = "#C6CE41", font_size = 12, font_family = "Poppins")
      ...
      # Value boxes
      value_box_content<- reactive(value_box_content_accueil_velo(filtered_data(), presta_types_table(), "epci", current_year,
...
# mod_accueil_velo_epci_ui("accueil_velo_epci_1")

## To be copied in the server
# mod_accueil_velo_epci_server("accueil_velo_epci_1")
