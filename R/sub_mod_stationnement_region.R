#' stationnement_region UI Function
#'
...
mod_stationnement_region_ui <- function(id, current_year){
  ns <- NS(id)
  tagList(
    br(),
...
#' stationnement_region Server Functions
#'
#' @noRd
mod_stationnement_region_server <- function(id, validated_selection, current_year, admin_cache) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      mod_intro_panel_server("intro_st", "stationnement",
                             ...
                             stationnement_regional, "stationnement_data")

      # Filter stationnement data by selected region
      filtered_data <- reactive(stationnement_regional |> filter(INSEE_REG == validated_selection$region))
      ...
      output$evolution_stat_hab <- renderPlotly({
        data_comparison <- generate_stationnement_comparison_data_local(data_cat_terr, stationnement_regional,
                                                                        "stat_hab", "region", validated_selection$region,
                                                                        2022, current_year)
        ...
        plot_evolution_stationnement(filtered_data(), data_comparison_cat, "stat_hab", territory_name(),
                                     2022, current_year, "region", lib_category)
      })

      # Evolution - Parking spots per km of cycling infrastructure
      output$evolution_stat_km <- renderPlotly({
        data_comparison <- generate_stationnement_comparison_data_local(data_cat_terr, stationnement_regional,
                                                                        "stat_km", "region", validated_selection$region,
                                                                        2022, current_year)
        data_comparison_cat <- data_comparison$data_comparison_cat
        lib_category <- data_comparison$lib_category
      ...
      output$stationnement_density <- renderPlotly({
        plot_stationnement_density(stationnement_communal, data_cat_terr,
                                   "region", validated_selection$region, territory_name(), current_year)
      })
...
# mod_stationnement_region_ui("stationnement_region_1")

## To be copied in the server
# mod_stationnement_region_server("stationnement_region_1")
