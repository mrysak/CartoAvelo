...
mod_veloroute_france_ui <- function(id, current_year){
  ns <- NS(id)
  tagList(
    br(),
          ...
          selectInput(ns("indicator_choice"), "Indicateur à visualiser",
                      choices = setNames(
                        ...
                        c("Taux de réalisation des différents schémas cyclables",
                          ...
                          "Cartographie de l'avancement des véloroutes du SNV")
                      ),
                      width = "88%"
          ),
          conditionalPanel(
            ...
            selectInput(ns("display_typology"), "Affichage par",
                        choices = setNames(c("region", "departement"),
                                           c("Régions", "Départements")),
                        width = "88%"
            )
          ),
          conditionalPanel(
      ...
      card_body(
        layout_columns(
          ...
          div(
            ...
            div(
              style = "display: flex; align-items: flex-start; justify-content: flex-start; gap: 25px;",
              plotlyOutput(ns("new_achievements_status"), height = "265px", width = "70%"),
              tags$div(
                class = "custom-legend",
                style = "font-family: Poppins; font-size: 15px; line-height: 1.4;",
    ...
    card(
      ...
      card_body(
        div(
          style = "margin-left: 180px; margin-right: 200px; margin-top: -20px;",
...
#' veloroute_france Server Functions
#'
#' @noRd
mod_veloroute_france_server <- function(id, validated_selection, current_year, veloroute_cache) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      output$plot_completion_ev <- renderPlotly({
        plot_avancement_veloroutes(veloroute_national, current_year, "EV",
                                   "france", "", "")
      })

      output$plot_completion_snv <- renderPlotly({
        plot_avancement_veloroutes(veloroute_national, current_year, "SNV-STRICT",
                                   "france", "", "")
      })

      output$plot_completion_srv <- renderPlotly({
        plot_avancement_veloroutes(veloroute_national, current_year, "SRV-STRICT",
                                   "france", "", "")
      })

      output$plot_completion_sdv <- renderPlotly({
        plot_avancement_veloroutes(veloroute_national, current_year, "SDV-STRICT",
                                   "france", "", "")
      })

      # Value box - Total number of kilometers
      vb_content_total <- reactive(value_box_content_veloroute(veloroute_national, current_year, "france",
                                    "", "total", ""))
      ...
      vb_content_status <- reactive(value_box_content_veloroute(veloroute_national, current_year, "france",
                                                               "", "status", ""))
      ...
      output$map_title <- renderText({
        ...
        label <- switch(input$indicator_choice,
                        ...
                        "avancement_maillage" = "Avancement des véloroutes du SNV"
        )
      ...
      output$map_veloroute_indicators <- renderLeaflet({
        ...
        if (input$indicator_choice %in% c("taux_realisation", "km_restant")){
          ...
          if (input$display_typology == "region"){
            ...
            veloroute_data <- veloroute_regional
          } else if (input$display_typology == "departement"){
            admin_sf <- admin_departement
            veloroute_data <- veloroute_departemental
          }

          info_to_display <- ifelse(input$indicator_choice == "taux_realisation", input$info_to_display, 'value')

          map_veloroute_indicators(data_cat_terr, veloroute_data, admin_sf, "france",
                                   input$display_typology, "", input$indicator_choice,
          ...
          map_network_progress_france(veloroute_sf, contour_metropole, session$ns)
        }
      })
      ...
      observeEvent(input$territoire_select, {
        ...
        leafletProxy("map_veloroute_indicators") %>%
      ...
      vb_content_map <- reactive({
        ...
        value_box_content_veloroute(veloroute_national, current_year, "france",
                                    "", params$vb_type, params$schema_type)
      })
      ...
      # New achievements
      plot_new_achievements <- reactive(plot_new_creations(veloroute_national, current_year,
                                                           "france", "", "France"))
      ...
      vb_content_new <- reactive(value_box_content_veloroute(veloroute_national, current_year, "france",
                                                               "", "new_achievements", ""))
      ...
      output$history <- renderPlotly({
        plot_veloroute_history(veloroute_national, current_year, 2020,
                               "france", "")
      })
    }
  )
...
# mod_veloroute_france_ui("veloroute_france_1")

## To be copied in the server
# mod_veloroute_france_server("veloroute_france_1")
