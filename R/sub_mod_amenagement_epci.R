...
mod_amenagement_epci_ui <- function(id, current_year){
  ns <- NS(id)
  tagList(
    br(),
...
#' amenagement_epci Server Functions
#'
#' @noRd
mod_amenagement_epci_server <- function(id, validated_selection, current_year, admin_cache) {
  moduleServer(
    id,
    function(input, output, session) {
      ...
      # Filter amenagement data by selected EPCI
      ...
      output$evolution_rate <- renderPlotly({
        data <- generate_evolution_rate_local(data_cat_terr, amenagement_epci, "epci",
                                              validated_selection$epci, territory_name(), time_period,
      ...
      output$evolution_voirie <- renderPlotly({
        comparison_data <- generate_comparison_data_local(data_cat_terr, amenagement_epci, "epci",
                                                          input$indicator, validated_selection$epci, time_period,
        ...
        plot_evolution_amenagement(data_amenagement(), data_comparison_cat, input$indicator,
                                   territory_name(), time_period, "epci", lib_category,
                                   input$secure_amenagement)
      })
      ...
      output$evolution_taux_cyclabilite <- renderPlotly({
        comparison_data <- generate_comparison_data_local(data_cat_terr, amenagement_epci, "epci",
                                                          "taux_cyclabilite", validated_selection$epci, time_period)
...
# mod_amenagement_epci_ui("amenagement_epci_1")
