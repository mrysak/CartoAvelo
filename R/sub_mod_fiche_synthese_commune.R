#' fiche_synthese_commune UI Function
#'
...
mod_fiche_synthese_commune_ui <- function(id, current_year_ac, current_year_st){
  ns <- NS(id)
  tagList(
    br(),
...
#' fiche_synthese_commune Server Functions
#'
#' @noRd
mod_fiche_synthese_commune_server <- function(id, validated_selection, current_year_ac, current_year_st, admin_cache) {
  moduleServer(
    id,
    function(input, output, session) {

      # Introduction panel
      mod_intro_panel_server_fiche_synthese("intro_fs", validated_selection, data_cat_terr, load_admin_sf("commune", admin_cache))
      ...
      data_stationnement <- reactive(stationnement_communal |> filter(INSEE_COM == validated_selection$commune))
      data_accueil_velo <- reactive(accueil_velo_communal |> filter(INSEE_COM == as.numeric(validated_selection$commune)))

      # Radar chart
      output$radar_chart <- renderPlotly({
        data <- generate_data_radar_chart(data_cat_terr, amenagement_communal, stationnement_communal,
                                          part_modale, veloroute_communal, accueil_velo_communal,
                                          current_year_ac, current_year_st, current_year_pm,
                                          current_year_vr, current_year_av, "commune", validated_selection$commune)
      ...
      comparison_data <- reactive(generate_comparison_data_local(data_cat_terr, amenagement_communal, "commune",
                                                                 "taux_cyclabilite", validated_selection$commune, seq(2021, current_year_ac)))
      ...
      output$evolution_taux_cyclabilite <- renderPlotly({
        ...
        plot_evolution_amenagement(data_amenagement(), data_comparison_cat, "taux_cyclabilite",
                                   territory_name(), seq(2021, current_year_ac), "commune", lib_category()) |>
      ...
      output$evolution_voirie_hab <- renderPlotly({
        comparison_data <- generate_comparison_data_local(data_cat_terr, amenagement_communal, "commune",
                                                          "voirie_hab", validated_selection$commune, seq(2021, current_year_ac))
      ...
      output$evolution_stat_hab <- renderPlotly({
        comparison_data <-generate_stationnement_comparison_data_local(data_cat_terr, stationnement_communal, "stat_hab",
                                                                       "commune", validated_selection$commune, 2022, current_year_st)

        data_comparison_cat <- comparison_data$data_comparison_cat

        plot_evolution_stationnement(data_stationnement(), data_comparison_cat, "stat_hab",
                                     territory_name(), 2022, current_year_st, "commune", lib_category()) |>
          layout(showlegend = FALSE, title = list(text = ""))
      })

      # Evolution - Stationnement par km d'aménagement
      output$evolution_stat_km <- renderPlotly({
        comparison_data <- generate_stationnement_comparison_data_local(data_cat_terr, stationnement_communal, "stat_km",
                                                                        "commune", validated_selection$commune, 2022, current_year_st)
      ...
      output$evolution_part_modale <- renderPlotly({
        data_evolution <- generate_pm_comparison_data_local(data_cat_terr, part_modale, current_year_pm, 2018,
                                                            "velo", "commune", validated_selection$commune)
      ...
      # Value boxes
      vb_pm_content <- reactive(value_box_content_pm(part_modale, current_year_pm, "commune", validated_selection$commune, "velo"))
      ...
      # Value boxes
      vb_av_content<- reactive(value_box_content_accueil_velo(data_accueil_velo(), presta_types_table(), "commune", current_year_av,
                                                              "count", "TOTAL", NULL))
...
# mod_fiche_synthese_commune_ui("fiche_synthese_commune_1")

## To be copied in the server
# mod_fiche_synthese_commune_server("fiche_synthese_commune_1")
